...
from core.example.generate_example import create_example_py_file

class TestExample(unittest.TestCase):

    def setUp(self):
        self.current_path = os.path.abspath(os.path.dirname(__file__))
        self.example_py_path = os.path.join(self.current_path, "test1.py")
        self.icon_folder_path = os.path.join(self.current_path, 'icon')
        self.temp_image_folder_path = os.path.join(self.current_path, 'test', 'temp_image')
