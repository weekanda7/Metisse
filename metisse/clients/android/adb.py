...
class AdbClient(Client):
    ...
    def __init__(self, device_params: DeviceParams):
        assert (
            device_params.os_environment == "android"
        ), "device_params.os_environment must be android"
    ...
    def screenshot(self, save_screenshot_path: str) -> None:
        ...
        """
        os.system(
            f"adb -s {self.device_params.device_id}  shell screencap -p /sdcard/screenshot.png"
        )
        os.system(
            f"adb -s {self.device_params.device_id}  pull /sdcard/screenshot.png {save_screenshot_path}"
        )
    ...
    def swipe(
        ...
        os.system(
            f"adb -s {self.device_params.device_id} shell input swipe {_x} {_y} {_x2} {_y2} {swiping_time}"
        )
