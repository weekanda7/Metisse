...
class MetisseClass(TemplateMetisClass):
    def __init__(
        ...
        self._logger = MetisseLogger(
            "MetisClass_logger",
            log_level=logging.DEBUG,
            log_file=os.path.join(
                self._script_path.device_id_path,
                "log",
    ...
    def check_gamelog(self, params: ImageRecognitionParams):
        ...
        try:
            ...
            self._img_recog_result = image_recognition.match_template(
                self.opencv_utils.screen_image_mat,
        ...
        if self._img_recog_result.is_recognized:
            self._logger.info(
                "match_template method : template_name=%s prob=%.4f accuracy_val=%.4f %s",
                params.template_image_name,
                self._img_recog_result.recognition_threshold,
                params.accuracy_val,
                self._img_recog_result.is_recognized,
    ...
    def check_image_recognition(self, params: ImageRecognitionParams) -> bool:
        ...
        def _single_time() -> bool:
            if (
                params.compare_times_counter > 1
                and params.is_refresh_screenshot is False
            ):
            ...
            for _num in range(params.compare_times_counter):
                ...
                if params.is_refresh_screenshot:
                    time.sleep(
                        params.screenshot_wait_time + self.screenshot_wait_time_increase
                    )
                    self.screenshot(
                        params.screen_image_name,
                        params.screen_image_primary_dir,
                        params.screen_image_secondary_dir,
                    )

                _screen_image_path = self._script_path.get_screen_image_path(params)
                _template_image_path = self._script_path.get_template_image_path(params)
                self.opencv_utils = Opencv_utils(
                    _screen_image_path, _template_image_path
                )
                self._img_recog_result = image_recognition.match_template(
                    self.opencv_utils.screen_image_mat,
                    self.opencv_utils.template_image_mat,
                ...
                self._logger.info(
                    "image_recognition method : template_name=%s  prob=%.4f accuracy_val=%.4f %s",
                    params.template_image_name,
                    self._img_recog_result.recognition_threshold,
                    params.accuracy_val,
                    self._img_recog_result.is_recognized,
        ...
        def _multiple_times() -> bool:
            _screen_image_name_list = [
                f"tmp{x}" for x in range(params.repeatedly_screenshot_times)
            ]
            if self.is_check_gamelog:
                self.check_gamelog(ImageRecognitionParams())
            _template_image_path = self._script_path.get_template_image_path(params)
            ...
            for _num in range(params.compare_times_counter):
                ...
                for _temp_screen_image_name in _screen_image_name_list:
                    ...
                    self._ui_client.send_image_path_to_ui(
                        _image_path=_template_image_path
                    )
                    self._ui_client.send_log_to_ui(
                        f"match_template method : \n template_name={params.template_image_name}  \n prob={self._img_recog_result.recognition_threshold:.4f} \n accuracy_val={params.accuracy_val:.4f} \n {self._img_recog_result.is_recognized}"
                    )
    ...
    def swipe(
        ...
        for _swipe_time in range(swipe_execute_counter_times):
            ...
            self._logger.info(
                "swipe method : (x,y) = %s (x2,y2) = %s swiping_time = %.2f",
                center,
    ...
    def press(
        ...
        for _swipe_time in range(press_execute_counter_times):
            try:
                ...
                self._logger.info(
                    "adb_press method : (x,y) = %s pressing_time = %d",
                    center,
                    pressing_time,
                )
                self._ui_client.send_log_to_ui(
